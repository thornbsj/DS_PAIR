...
if __name__ == '__main__':
    try:
        ...
        rouge = Rouge()
        def compute_metric_rouge(evalPredict):
            predictions,labels = evalPredict
            decode_preds = tokenizer.batch_decode(predictions,skip_special_tokens=True)
            labels = np.where(labels!=-100,labels,tokenizer.pad_token_id)
            decode_labels = tokenizer.batch_decode(labels,skip_special_tokens=True)
            decode_preds = [" ".join(x) for x in decode_preds]
            decode_labels = [" ".join(x) for x in decode_labels]
            try:
                scores = rouge.get_scores(decode_preds,decode_labels,avg=True)
                return {"rouge-l":scores["rouge-l"]["f"]}
            except:
                return {"rouge-l":np.inf}

        def compute_metric_bleu(evalPred):
            predictions, labels = evalPred
            decode_preds = tokenizer.batch_decode(predictions, skip_special_tokens=True)
            labels = np.where(labels != -100, labels, tokenizer.pad_token_id)
            decode_labels = tokenizer.batch_decode(labels, skip_special_tokens=True)
            # decode_preds = [p for p in decode_preds]
            # decode_labels = [l for l in decode_labels]
            scores = sentence_bleu(decode_labels, decode_preds)
            return {
                "bleu": scores
            }
        ...
        trainer = Seq2SeqTrainer(model=model,args=train_args,train_dataset=tokenized_data,
                data_collator = DataCollatorForSeq2Seq(tokenizer=tokenizer),
                tokenizer=tokenizer,compute_metrics=compute_metric_bleu if args.eval_metric == "bleu" else compute_metric_rouge,
